...
6. Перед изменением agent runtime, tools, prompt или UI установи, что уже
...
Подробная постановка пользователя, ограничения данных и критерии успеха
описаны в [`docs/PROJECT_CONTEXT.md`](docs/PROJECT_CONTEXT.md). При расхождении
старого UX «сначала ИНН — затем отчёт» с этим файлом новая agent-first точка
входа является целевым направлением, а старый отчёт сохраняется как capability
и регрессионный сценарий.
