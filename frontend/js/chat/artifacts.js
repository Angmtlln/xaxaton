...
function renderCompanyShortlist(block) {
  ...
  rows.forEach((row) => {
    ...
    line.appendChild(element('td', 'comparison-cell', String(row.enforcement_count ?? 'Нет данных')));
    const stops = row.hard_stops;
    const stopCell = element('td', `comparison-cell shortlist-stops-${stops ? 'yes' : 'no'}`);
    stopCell.appendChild(element('span', null, stops == null ? 'Нет данных' : stops ? String(stops) : 'нет'));
