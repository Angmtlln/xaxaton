...
def _collect(inn: str, parts: list[TargetedData]) -> tuple[ComparisonCompanyData, dict, list]:
    ...
    for part in parts:
        ...
        metric_ids.extend(_prefixed(inn, item) for item in part.metric_ids)
    ...
    company = ComparisonCompanyData(
        ...
        # В таблицу попадают только сравнимые меры, остальное остаётся в фактах.
        metric_ids=[item for item in metric_ids if measure_key(item.split(":", 1)[1])][:16],
        status_ids=status_ids[:8],
        policy_signal_ids=signal_ids[:8],
