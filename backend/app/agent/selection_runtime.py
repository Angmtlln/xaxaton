...
from .prompt import FOLLOW_UP_INSTRUCTIONS, MASTER_PROMPT_VERSION
...
def handles_selection(message, previous):
    ...
    return bool(
        ...
        and re.search(r"поставщик|для\s+(?:регулярных\s+)?постав(?:ок|ки)|для\s+закуп\w*|покупател|партн[её]р|сотруднич|подходящ|над[её]ж|качественн|глубок|лучших\s+(?:контрагент|компани)|лучшие\s+(?:контрагент|компани)", message, re.I)
    )
