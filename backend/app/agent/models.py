...
class FindCompaniesArgs(StrictModel):
    ...
    max_profit: Optional[float] = Field(default=None, allow_inf_nan=False, description='Прибыль до указанной суммы, в рублях.')
    ...
    min_claims_amount: Optional[float] = Field(default=None, ge=0, allow_inf_nan=False, description='Сумма исков к ответчику от указанной суммы, в рублях.')
...
class ShortlistRow(StrictModel):
    ...
    enforcement_count: Optional[int] = Field(default=None, ge=0)
    hard_stops: Optional[int] = Field(default=None, ge=0)
    risk_level: SafeText
