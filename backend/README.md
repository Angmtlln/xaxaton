...
| Итоговое summary | `openai/gpt-oss-120b` | один вызов, собирает четыре блока в экран ответа |
...
Оформление взято из прототипа фронтендера
[alfa-counterparty-prototype](../alfa-counterparty-prototype). Прототип
собран на Next и Tailwind, но вся его стилистика лежит обычным CSS в
`app/globals.css`, поэтому этот файл перенесён в
[static/styles.css](static/styles.css) без правок, а сверху добавлен
базовый слой (шрифт, кнопки `ProductButton`, `.sr-only`) и наши
дополнения в той же идиоме: без скруглений, границы 1px, заголовки капсом.

Разметка повторяет прототип покомпонентно: красный лендинг с крупным
заголовком, отчёт с липкой боковой навигацией, тёмная сводка
`summary-panel` с темой по итоговой группе, карточки направлений
`risk-card` на `<details>` и строки доказательств `evidence-row` с чипом
типа и путём в исходном JSON.
