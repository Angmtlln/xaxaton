...
По умолчанию `WEB_NEWS_ENABLED=true`. Для конфиденциальных условий сделки
установить `WEB_NEWS_ENABLED=false` и перезапустить backend: web plugin не
передаётся, публикации не загружаются, статус — `not_configured`. Это отключение
внешнего поиска, а не режима LLM: текст по-прежнему передаётся настроенному
провайдеру модели. При включённом plugin изоляция его поискового запроса от
контекста не гарантируется; см. [границы защиты](security/DEMO_HARDENING_2026-09-07.md).
...
Frontend отображает массив горизонтальной полосой «Важные новости» внизу
полного ответа: дата, источник, заголовок и описание. Новости листаются вручную,
ссылки открываются безопасно, текст не интерпретируется как HTML. Renderer и
UX-состояния описаны в [CHAT_UI.md](CHAT_UI.md).
