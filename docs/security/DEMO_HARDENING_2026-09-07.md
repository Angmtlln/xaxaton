...
| S3: цвета модели | По последующему явному запросу пользователя цветное мнение AI восстановлено. Backend сохраняет unknown при пробелах и override официального hard stop. Это осознанное продуктовое исключение; остальные защиты сохранены. |
...
как раньше. Роза снова показывает качественное мнение AI, увеличена на desktop
на 15%; меню под ней удалено. Пояснения — при наведении на секторы. Это не
банковская оценка: ограничения источника и отсутствие данных проверяет backend.
...
- Профиль: цветное мнение AI, unknown при пробелах, официальный стоп-сигнал
  нельзя смягчить; банковские значения совпадают с источником.
