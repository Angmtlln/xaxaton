...
MCP реализован как отдельный интерфейс чтения между backend и PostgreSQL.
Docker использует его по умолчанию; direct — явный режим отладки/отката.
LangChain, domain tools и расчёт фактов сохранены, новой LLM-стадии нет.
Запись аудита остаётся отдельным DB-механизмом.
[Контракты и запуск MCP](MCP_DATA_ACCESS.md). Desktop — текущий приоритет;
