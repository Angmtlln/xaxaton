...
Done:
- provider-neutral Master factory: OpenRouter, Polza and Groq;
- ordered spare models with the pause named by the provider, so one exhausted
  model does not end the turn in a canned answer;
...
Open:
- OpenRouter + GLM-5.3-Flash on a real key;
- seven-turn conversation with zero fallback on a data-rich company.

The Groq free tier caps output tokens per model per minute and per day, so the
stage closes on a provider without that cap rather than on more retry logic.
