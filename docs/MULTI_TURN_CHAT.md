...
При заполнении store новые диалоги получают `conversation_capacity`; неистёкшие
диалоги не вытесняются. Продолжение существующего ID остаётся доступным.
До runtime действуют [лимиты API](security/DEMO_HARDENING_2026-09-07.md).

Системная роль содержит инструкции/схему и проверенные ИНН. Данные текущего
инструмента передаются в ToolMessage, кэшированные observations — отдельным
сообщением данных в user-роли без фиктивного tool call. Сохранённые пользовательские
условия также остаются в user-роли. Полный нормализованный контекст передаётся
один раз; source/user-текст не вставляется в system prompt.
